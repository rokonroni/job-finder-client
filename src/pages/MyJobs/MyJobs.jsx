  ...
  if (myJobs.length > 0) {
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3 my-5 container mx-auto">
        {
             myJobs.map((job) => <MyJob key={job._id} job={job} myJobs={myJobs} setMyJobs={setMyJobs} />
        )}
      </div>
    </>
  );
    ...
    return (
      ...
      >
        <div className="flex flex-col justify-center items-center">
          <strong className="font-bold">Error: <span className="block sm:inline"> No Jobs Added Yet</span></strong>
          <Link to="/addJob">
            <button className="btn btn-info mt-5 text-white font-bold">Add a job</button>
          </Link>
        </div>
      </div>
        </>
